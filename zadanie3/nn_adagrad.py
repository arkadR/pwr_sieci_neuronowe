...
class NeuralNetworkAdagrad:
    ...
    def __init__(self, sizes, W, b, activations):
        ...
        self.best_val_acc = 0
        # Adagrad
        self.dW_sq_his = [w * 0 for w in self.W]
        self.db_sq_his = [b * 0 for b in self.b]
        # self.mean_dW = [w * 0 for w in self.W]
        # self.mean_db = [b * 0 for b in self.b]

    @staticmethod
    def from_file(path):
        with open(path, 'rb') as file:
            npz = np.load(file, allow_pickle=True)
            return NeuralNetworkAdagrad(npz['sizes'], npz['W'], npz['b'], npz['activations'])
    ...
    def create_random(sizes, activations, weight_range = 0.01, bias_range = 0.01):
        ...
        return NeuralNetworkAdagrad(sizes, W, b, activations)
    ...
    def __update_parameters(self, dW, db, eta):
        self.dW_sq_his += np.power(dW, 2)
        self.db_sq_his += np.power(db, 2)
        W_update = []
        b_update = []
        for i, _ in enumerate(self.W):
            W_update.append((eta * dW[i]) / (np.sqrt(self.dW_sq_his[i]) + 0.00001))
            b_update.append((eta * db[i]) / (np.sqrt(self.db_sq_his[i]) + 0.00001))
        for i, _ in enumerate(self.W):
            self.W[i] -= W_update[i]
            self.b[i] -= db[i]
        # self.
        # count = len(self.mean_dW)
        # for i, _ in enumerate(self.mean_dW):
            # self.mean_dW[i] = self.mean_dW[i] * ((count-1) / count) + (np.power(dW[i], 2) / count)
            # self.mean_db[i] = self.mean_db[i] * ((count-1) / count) + (np.power(db[i], 2) / count)
