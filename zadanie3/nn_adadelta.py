...
class NeuralNetworkAdadelta:
    ...
    def __init__(self, sizes, W, b, activations):
        ...
        self.best_val_acc = 0
        # Adadelta
        self.E_g_W = [w * 0 for w in self.W]
        self.E_g_b = [b * 0 for b in self.b]
        self.E_d_W = [w * 0 for w in self.W]
        self.E_d_b = [b * 0 for b in self.b]

    @staticmethod
    def from_file(path):
        with open(path, 'rb') as file:
            npz = np.load(file, allow_pickle=True)
            return NeuralNetworkAdadelta(npz['sizes'], npz['W'], npz['b'], npz['activations'])
    ...
    def create_random(sizes, activations, weight_range = 0.01, bias_range = 0.01):
        ...
        return NeuralNetworkAdadelta(sizes, W, b, activations)
    ...
    def __update_parameters(self, dW, db, eta):
        ...
        for i, _ in enumerate(dW):
            self.E_g_W[i] = y * self.E_g_W[i] + (1-y) * np.square(dW[i])
            self.E_g_b[i] = y * self.E_g_b[i] + (1-y) * np.square(db[i])
            W_change = -np.sqrt((self.E_d_W[i] + e) / (self.E_g_W[i] + e)) * dW[i]
            b_change = -np.sqrt((self.E_d_b[i] + e) / (self.E_g_b[i] + e)) * db[i]
            self.W[i] += W_change
            self.b[i] += b_change
            self.E_d_W[i] = y * self.E_d_W[i] + (1-y) * np.square(W_change)
            self.E_d_b[i] = y * self.E_d_b[i] + (1-y) * np.square(b_change)
