...
class NeuralNetworkNestrov:
    ...
    def from_file(path):
        with open(path, 'rb') as file:
            ...
            return NeuralNetworkNestrov(npz['sizes'], npz['W'], npz['b'], npz['activations'])
    ...
    def create_random(sizes, activations, weight_range = 0.01, bias_range = 0.01):
        ...
        return NeuralNetworkNestrov(sizes, W, b, activations)
    ...
    def __activation_error(self, Y, Z, A):
        ...
        W_mod = self.W if self.prev_dW == 0 else self.W - self.prev_dW * self.momentum
        for i, act in reversed(list(zip(range(len(dA)-1), self.activation_primes))):
            dA[i] = W_mod[i+1].T.dot(dA[i+1])*act(Z[i])
        return dA
